...
export const alertVariants = cva(
  'relative w-full rounded-md overflow-hidden p-3 bg-elevated-on-dark text-base grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-2 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-secondary',
  {
    variants: {
      variant: {
